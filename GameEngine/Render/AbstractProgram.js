define(["require", "exports", "./gl/GLManager"], function (require, exports, GLManager_1) {
    ...
    class AbstractProgram {
        constructor(name, vertexSource, fragmentSource) {
            ...
            this.program = GLManager_1.gl.createProgram();
            if (this.program !== null && this.vertexShader !== null && this.fragmentShader !== null) {
                this.compileProgram(this.vertexShader, this.fragmentShader, this.program);
            }
            else {
        ...
        useProgram() {
            GLManager_1.gl.useProgram(this.program);
        }
        detectAttributes() {
            if (this.program != null) {
                let attributeCount = GLManager_1.gl.getProgramParameter(this.program, GLManager_1.gl.ACTIVE_ATTRIBUTES);
                for (let i = 0; i < attributeCount; i++) {
                    let attributeInfo = GLManager_1.gl.getActiveAttrib(this.program, i);
                    if (!attributeInfo) {
                        break;
                    }
                    this.attributes[attributeInfo.name] = GLManager_1.gl.getAttribLocation(this.program, attributeInfo.name);
                }
            }
            else {
        ...
        detectUniforms() {
            if (this.program !== null) {
                let uniformCount = GLManager_1.gl.getProgramParameter(this.program, GLManager_1.gl.ACTIVE_UNIFORMS);
                for (let i = 0; i < uniformCount; i++) {
                    let uniformInfo = GLManager_1.gl.getActiveUniform(this.program, i);
                    if (!uniformInfo) {
                        break;
                    }
                    console.log("uniform detected");
                    this.uniforms[uniformInfo.name] = GLManager_1.gl.getUniformLocation(this.program, uniformInfo.name);
                }
            }
            else {
